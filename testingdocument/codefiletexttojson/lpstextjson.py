...
# Load invoice text
# -------------------------
input_path = "./txtfile/LSPL 356 Irillic Private Limited_02.txt"  # ✅ Change this to your file path
...
buyer_address = extract(rf"Buyer\n{re.escape(buyer_name)}\n(.+\n.+\n.+)", text, '').replace("\n", ", ")
...
for i, line in enumerate(lines):
    if line.strip() in invoice_keys:
        ...
        invoice_details[key] = next_line.strip() if next_line.strip() and next_line.strip() not in invoice_keys else ""
...
while i < len(lines):
    line = lines[i].strip()
    if re.match(r"^\d+\s+Supply of Prototype Parts", line) or re.match(r"^\d+\s+Accounting Services", line):
...
# Save to file
with open("LSPL 356 Irillic Private Limited_02.json", "w", encoding="utf-8") as f:
    json.dump(output_data, f, indent=4)

print("✅ Invoice parsed and saved to invoice_output.json")
